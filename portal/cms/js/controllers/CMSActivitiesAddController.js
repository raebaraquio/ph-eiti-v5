var scope;
cms.controller('CMSActivitiesAddController',['$scope','NewsFactory','$location','$sce','UploadFactory','CMSFactory','sessionService',
	...
	$scope.steps = ["Duration","Details","Upload","Write Up","Save"]
	$scope.activeStory = "Duration"
	$scope.continueOpts = ["Continued","Continue reading","Read more"]
	$scope.activity = {}
	$scope.onedayactivity = null
	...
	$scope.traverse = function(v) {
		...
		if (v=='Details') {
			if ($scope.onedayactivity===null || $scope.onedayactivity===undefined) {
				$scope.toggleInlineFeedback('show','error', 'Unable to proceed. Please select from the choices below.','add');
				return false
			}
		}
		
		if (v!=='Details' && v!=='Duration') {
			if (!$scope.activity.pk) {
	...
	$scope.validateForm = function(currForm,nextForm){
		...
		switch(currForm) {
			case 'Details':
				...
				$scope.activity.id = $scope.activity.title
				if ($scope.activity.title.match(/ /i)) {
					var title = $scope.activity.title.split(' ');
					$scope.activity.id = title.join('-');	
				}
				
				// check for: source url, source, date posted
				if ($scope.onedayactivity===false) {
					if (!$scope.activity.sourceurl) {
						$scope.setInvalid(fieldId+'sourceurl')
						$scope.toggleInlineFeedback('show','error', 'Source URL is required.','add');
						return false		
					}
					if (!$scope.activity.source) {
	...
	$scope.save = function(currForm,nextForm){
		...
		switch(currForm) {
				...
				$scope.savePromise.then(function(response){
					if (response.data) {
						if (response.data.newspk) {
							$scope.activity.pk = response.data.newspk
							$scope.traverse(nextForm);
							$scope.setElemValue('newsfk',$scope.activity.pk)
						}
					}
				}, function(err){
					console.log(err)
			...
			case 'Save':
				// Update promise
				$scope.activity.publish = "publish"
				$scope.savePromise = CMSFactory.news.update($scope.entry)
				$scope.savePromise.then(function(response){
					if (response.data) {
						if (response.data.success) {
