...
if (isset($_POST['programform-submit'])) {
  
	$activity_id_fk = NULL;
    if (isset($_POST['programFile-activityidfk']) && !(empty($_POST['programFile-activityidfk']))) {
        $activity_id_fk = intval(mysql_escape_string(trim(strip_tags($_POST['programFile-activityidfk']))));
    }
    $href = "";
    if (isset($_POST['programFile-hreftitle']) && !(empty($_POST['programFile-hreftitle']))) {
        $href = mysql_escape_string(trim(strip_tags($_POST['programFile-hreftitle'])));
    }
    $u = new Upload();

    $curr_file_id = 'programfile';
